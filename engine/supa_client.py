...
class SupabaseClient:
    """
    一个用于操作 Supabase 的异步 CRUD 封装类。
    支持：查询 / 条件查询 / 插入 / 批量插入 / 更新 / 删除 / upsert / 批量 upsert。

    使用工厂方法创建实例：
        client = await SupabaseClient.create(url, key)
    """

    def __init__(self):
        """私有构造函数，请使用 create() 类方法创建实例"""
        self.url: str | None = None
        self.key: str | None = None
        self.client: AsyncClient | None = None

    @classmethod
    async def create(cls, url: str | None = None, key: str | None = None) -> "SupabaseClient":
        """
        异步工厂方法：创建并初始化 SupabaseClient 实例
        ...
        instance.url = url
        instance.key = key
        if not instance.url or not instance.key:
            raise ValueError("缺少 SUPABASE_URL 或 SUPABASE_KEY")
        instance.client = await acreate_client(instance.url, instance.key)
        return instance

    # ========================== 查询 ==========================
    async def _get_records(self, table: str, filters: dict) -> list[dict] | None:
        """
        通用方法：根据条件从指定表获取多条记录
    ...
    async def _get_single_record(self, table: str, filters: dict) -> dict | None:
        """
        通用方法：根据条件从指定表获取单条记录
        ...
        """
        records = await self._get_records(table, filters)
        return records[0] if records else None

    # 冒险者相关
    async def get_adventurer_id_by_way_number(self, way: str, number: str) -> str | None:
        rec = await self._get_single_record(
            "adventurer", {"contact_way": way, "contact_number": number}
        )
        return rec["id"] if rec else None

    async def get_adventurer_by_way_number(self, way: str, number: str) -> Adventurer | None:
        rec = await self._get_single_record(
            "adventurer", {"contact_way": way, "contact_number": number}
        )
        return Adventurer.from_dict(rec) if rec else None

    async def get_adventurer_by_id(self, id: str) -> Adventurer | None:
        rec = await self._get_single_record("adventurer", {"id": id})
        return Adventurer.from_dict(rec) if rec else None

    # 委托人相关
    async def get_clienter_id_by_way_number(self, way: str, number: str) -> str | None:
        rec = await self._get_single_record(
            "clienter", {"contact_way": way, "contact_number": number}
        )
        return rec["id"] if rec else None

    async def get_clienter_by_way_number(self, way: str, number: str) -> Clienter | None:
        rec = await self._get_single_record(
            "clienter", {"contact_way": way, "contact_number": number}
        )
        return Clienter.from_dict(rec) if rec else None

    async def get_clienter_by_id(self, id: str) -> Clienter | None:
        rec = await self._get_single_record("clienter", {"id": id})
        return Clienter.from_dict(rec) if rec else None
    ...
    async def get_quest_by_id(self, quest_id: str) -> Quest | None:
        ...
        """
        rec = await self._get_single_record("quest", {"id": quest_id})
        return Quest.from_dict(rec) if rec else None

    async def get_quests_by_clienter_id(self, clienter_id: str) -> list[Quest] | None:
        """
        ...
        """
        rec = await self._get_records("quest", {"clienter_id": clienter_id})
        return Quest.from_list(rec) if rec else None

    # 状态相关
    async def get_adventurers_by_status(
        ...
        """
        # 获取所有状态符合的冒险者记录
        records = await self._get_records("adventurer", {"status": status.value})
        return Adventurer.from_list(records) if records else None

    async def get_available_quests(self) -> list[Quest] | None:
        """
        ...
        ongoing_assigns = await self.get_quest_assigns_by_status(QuestAssignStatus.UNANSWERED)
        ...
        for assign in ongoing_assigns:
            quest_dict = await self._get_single_record("quest", {"id": assign.quest_id})
            if quest_dict:
                available_quests.append(Quest.from_dict(quest_dict))

        return available_quests if available_quests else None

    async def get_active_quest_assign_by_adventurer(
    ...
    async def get_quest_assigns_by_status(
        ...
        """
        records = await self._get_records("quest_assign", {"status": status.value})
        return QuestAssign.from_list(records) if records else None

    # ========================== 插入 ==========================
    # 冒险者相关
    async def insert_adventurer(self, adventurer: Adventurer) -> bool:
        """
        ...
        """
        try:
            res = await self.client.table("adventurer").insert(adventurer.to_dict()).execute()
    ...
    # 委托人相关
    async def insert_clienter(self, clienter: Clienter) -> bool:
        """
        将 Clienter 实例插入到数据库
        ...
        """
        try:
            res = await self.client.table("clienter").insert(clienter.to_dict()).execute()
            if hasattr(res, "data") and res.data:
                return True
            else:
    ...
    # 委托相关
    async def insert_quest(self, quest: Quest) -> bool:
        """
        将 Quest 对象插入数据库。
        ...
        try:
            d = quest.to_dict()
            res = await self.client.table("quest").insert(d).execute()
    ...
    # 冒险者相关
    async def update_adventurer(self, adventurer: Adventurer) -> bool:
        """
        更新冒险者信息到数据库
    ...
    # 委托人相关
    async def update_clienter(self, clienter: Clienter) -> bool:
        """
        更新委托人信息到数据库
    ...
    async def insert_quest_assign(self, quest_assign: QuestAssign) -> bool:
        """插入任务分配记录"""
        try:
            res = await (
    ...
    async def update_quest_assign(self, quest_assign: QuestAssign) -> bool:
        """更新任务分配记录"""
    ...
    async def get_quest_assign_by_adventurer_status(
    ...
    async def get_quest_assigns_by_quest_id(self, quest_id: str) -> list[QuestAssign] | None:
        """获取某任务的所有分配历史"""
        records = await self._get_records("quest_assign", {"quest_id": quest_id})
        return QuestAssign.from_list(records) if records else None

    # ========================== quest_material operations ==========================
    async def insert_quest_material(self, quest_material: QuestMaterial) -> bool:
        """
        插入任务材料记录
    ...
    async def get_quest_materials_by_quest_id_type(
    ...
    async def insert_system_log(self, log: SystemLog) -> bool:
        """插入系统日志"""
        try:
            res = await self.client.table("system_log").insert(log.to_dict()).execute()
            return bool(getattr(res, "data", None))
        except Exception as e:
            logger.error(f"插入系统日志失败: {e}")
            return False

    async def log_event(self, event: str, detail: str | None = None) -> bool:
        """便捷方法：记录系统事件"""
        log = SystemLog(event=event, detail=detail)
        return await self.insert_system_log(log)
